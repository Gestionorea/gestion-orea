...
export default async function EditPropertyPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  return <div className="py-8"><p className="text-xs font-medium uppercase tracking-[0.25em] text-gray-500">{t('eyebrow')}</p><h1 className="mt-3 font-serif text-3xl tracking-[0.08em] text-black">{t('editTitle')}</h1><PropertyForm property={property} companies={companies} /></div>;
}
