...
import {
  ...
  setPaymentSourceOwners,
  type CoOwnerInput,
} from '@/lib/payment-source-owners';
...
export async function setPaymentSourceOwnersAction(formData: FormData): Promise<OwnershipActionResult> {
  await requireOwner();

  const paymentSourceId = formData.get('paymentSourceId');
  if (typeof paymentSourceId !== 'string' || !paymentSourceId) {
    return { ok: false, error: 'paymentSourceId requis' };
  }
  ...
  try {
    await setPaymentSourceOwners(paymentSourceId, owners);
  } catch (error) {
    if (error instanceof OwnershipValidationError) return { ok: false, error: error.message };
    return { ok: false, error: 'Echec mise a jour' };
  }

  revalidatePath('/fr/perso/admin/sources-paiement');
  revalidatePath('/en/perso/admin/sources-paiement');
